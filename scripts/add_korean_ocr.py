...
* ``labels.txt`` — the character table, one symbol per line, straight from
  PaddleOCR's ``korean_dict.txt``. The manifest reads it via
  ``postprocess.charset: labels``.
...
def fetch(work: Path) -> tuple[Path, list[str]]:
    ...
    chars = [ln for ln in _get(DICT_URL).decode("utf-8").splitlines() if ln.strip()]
