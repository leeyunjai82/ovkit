...
ovkit이 지금 할 수 있는 일 전부입니다. 조합 기능 19개, 모델 67개(추천 35개),
...
| `read_text` | 글자읽기 | `'STOP AHEAD'` — 읽는 순서대로 | 글자검출 + 글자인식 |
...
| 모델 | 무엇 |
| ---- | ---- |
| `text_recognition_0014` | 잘라낸 단어 읽기(CTC) |
...
| | |
| --- | --- |
| 한국어 OCR | 글자 인식은 영문 · 숫자 기준입니다. 한글 간판은 아직 못 읽습니다 |
