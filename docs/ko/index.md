...
| `count` | 무엇이 몇 개 있는지 | 검출 + 종류별 집계 |
| `posture` | 목이 계속 굽으면 경고 | 자세 + 목 각도, 시간축 |
| `exercise` | 스쿼트·팔굽혀펴기 횟수 | 자세 + 관절 각도 히스테리시스 |
| `attendance` | 누가 왔는지, CSV 출석부로 | 얼굴 검출 + 명단 매칭 |
| `teach` | 예시 사진으로 만드는 내 분류기 | 임베딩 + 코사인 k-NN (5모드) |
| `scene` | 사진 한 장을 한 문장으로 | 검출 + 분할 + 얼굴 |
| `read_plate` | 번호판과 그 차의 종류·색 | 번호판 검출 + OCR + 차량 속성 |
| `drowsiness` | 눈 감은 시간, 고개 끄덕임 | 얼굴 + 랜드마크 + 눈 상태 + 머리자세, 시간축 |
| `gesture` | 움직임에서 손동작 | 수어 모델 + 8프레임 롤링 버퍼 |
| `attention` | 사람이 어떤 물체를 보는지 | 시선 + 객체 검출 |
| `anonymize` | 얼굴(·번호판) 지운 사진 | 검출 + 모자이크 |
